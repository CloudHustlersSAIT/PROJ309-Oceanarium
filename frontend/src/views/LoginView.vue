      ...
      <main class="flex items-center justify-center px-4 py-3 md:py-4 overflow-hidden">
        <div class="w-full max-w-[620px]">
          ...
          <div class="bg-white shadow-xl rounded-none border border-black/5 p-6 sm:p-7 min-h-[620px]">
            ...
            <!-- Loading -->
            <p v-if="loading" class="text-sm text-black/70 mb-4">
              Checking auth status...
            </p>

            <!-- Form -->
            <form
              v-else-if="!firebaseDisabled"
              class="space-y-4"
            ...
            >
              ...
              <div class="space-y-1.5">
                ...
                <div v-if="mode === 'signup'" class="mt-2 text-xs space-y-1 text-black/70">
                  <div class="flex items-center gap-2" :class="hasMinLen ? 'text-emerald-700 font-medium' : 'text-black/70 font-normal'">
                    <span class="inline-flex h-4 w-4 items-center justify-center text-xs font-bold">
                      {{ hasMinLen ? "✓" : "•" }}
                    </span>
                    <span>At least 8 characters</span>
                  </div>
                  <div class="flex items-center gap-2" :class="hasSpecial ? 'text-emerald-700 font-medium' : 'text-black/70 font-normal'">
                    <span class="inline-flex h-4 w-4 items-center justify-center text-xs font-bold">
                      {{ hasSpecial ? "✓" : "•" }}
                    </span>
              ...
              >
                <span class="inline-flex items-center justify-center h-5 w-5">
                  <svg viewBox="0 0 48 48" class="h-5 w-5" aria-hidden="true">
                    <path fill="#EA4335" d="M24 9.5c3.54 0 6.31 1.53 7.76 2.81l5.66-5.66C33.98 3.64 29.47 1.5 24 1.5 14.73 1.5 6.79 6.82 3.06 14.56l6.89 5.35C11.66 14.05 17.35 9.5 24 9.5z"/>
                    <path fill="#4285F4" d="M46.5 24.5c0-1.64-.15-3.21-.43-4.73H24v9.01h12.64c-.55 2.95-2.21 5.45-4.7 7.13l7.25 5.63C43.66 37.43 46.5 31.52 46.5 24.5z"/>
                    <path fill="#FBBC05" d="M9.95 28.91c-.48-1.43-.75-2.95-.75-4.41s.27-2.98.75-4.41l-6.89-5.35C1.74 17.57 1 20.97 1 24.5s.74 6.93 2.06 9.76l6.89-5.35z"/>
                    <path fill="#34A853" d="M24 47.5c5.47 0 10.06-1.81 13.41-4.93l-7.25-5.63c-2.01 1.35-4.59 2.14-6.16 2.14-6.65 0-12.34-4.55-14.05-10.41l-6.89 5.35C6.79 42.18 14.73 47.5 24 47.5z"/>
                  </svg>
                </span>
...
async function handleSubmit() {
  ...
  try {
    ...
    else if (code === "auth/email-already-in-use") localError.value = "An account with this email already exists.";
    else if (code === "auth/invalid-email") localError.value = "Please enter a valid email address.";
    else if (code === "auth/weak-password") localError.value = "Password is too weak. Please use a stronger password.";
...
async function handleGoogleSignIn() {
  ...
  try {
    ...
    const code = err?.code || "";
    if (code === "auth/popup-closed-by-user") localError.value = "Google sign-in was closed before finishing.";
    else if (code === "auth/operation-not-allowed") localError.value = "Google sign-in is not enabled in Firebase Console.";
