...
| `requested_language_code` | String | nullable | ISO language code requested by the customer (e.g. `en`, `pt`) |
...
| `status` | String(50) | NOT NULL | Booking status (`unassigned`, `assigned`, `cancelled`, etc.) |
...
**Relationships:** many-to-many with `languages` via `guide_languages`, many-to-many with `tours` via `guide_tour_types`; has one `availability_patterns`; has many `schedules`, `surveys`
...
Maps which tours a guide is qualified to lead.
...
When a booking is created or updated, the system tries to auto-assign a guide. A guide is eligible only if **all** rules pass:

1. **Availability** -- guide has an active availability slot for the booking day, no blocking exception, and no overlapping schedule.
2. **Tour Type** -- guide is linked to the booking's tour via `guide_tour_types`.
3. **Language** -- guide speaks the `requested_language_code` on the booking via `guide_languages`.

Among eligible guides, the one with the fewest assignments on the target date is selected.
